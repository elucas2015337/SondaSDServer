        ...
        try {           
            ...
            var connection = new pSqlInstance.Connection(pSqlConfig, function (err) {
                ...
                request1.execute('[SWIFT_SP_GET_NEXT_PICKING_LOCATION]', function (err, recordsets, returnValue) {
                    if (returnValue == 0) {
                        socket.emit('get_next_location_ok', 
                        { 'sku': data.sku, 
                          'warehouse': recordsets[0][0].WAREHOUSE, 
                          'location': recordsets[0][0].LOCATION, 
                          'location_type': recordsets[0][0].LOCATION_TYPE 
                        });                        
                    }
                    else {
                        socket.emit('get_next_location_fail', 
                        { 'sku': data.sku, 
                          'error': returnValue, 
    ...
    function (data, socket) {
        try {
            ...
            var connection = new pSqlInstance.Connection(pSqlConfig, function (err) {
                ...
                request3 = new pSqlInstance.Request(connection);
        ...
        try {
            ...
            var connection = new pSqlInstance.Connection(pSqlConfig, function (err) {
                ...
                request.execute('[SWIFT_SP_CANCEL_SCAN]', function (err, recordsets, returnValue) {
                    ...
                    {
                        socket.emit('cancel_scan_fail', 
                        { 'error': err, 
                          'result': request.parameters.pRESULT.value 
                         }
                        );                    
                    }
                    connection.close();
