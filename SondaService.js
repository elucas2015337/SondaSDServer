...
module.exports = {
    ...
    GetBankAccounts: function (data, socket) {
        return new Promise((resolve, reject) => {
            try {
                ...
                var connection = _cnnServ.ConnectDb(data, function (err, pSqlInstance) {
                    var rowscount = 0;
                    console.log("getbankaccounts.1: " + data.routeid, "info");
                    ...
                    request.query(pSql, function (err, recordset) {
                        if (err) {
                            console.log(
                                "GetBankAccounts.query:" + JSON.stringify(err),
                                "error"
                            );
                            socket.emit("error_message", {
                ...
                console.log("GetBankAccounts.catch" + e.message, "error");
    ...
    SendAcceptedTransfer: function (data, socket) {
        try {
            InsertPosSkusOutsideRoute(data, socket, function (parameter, socketEmit, listaSkus, listasConversiones, connection, pSqlInstance) {
                var request = new pSqlInstance.Request(connection);
    ...
    UpdateUserImage: function (data, socket, errCallback) {
        try {
            ...
            errCallback(
                "Error al actualizar la imagen del operador: " + e.message
            );
        }
    }
};
   ...
   //Get SKU XML form SONDA SD and insert the registers on SONDA_POS_SKU
	try {
		...
		request.input("TRANSFER_ID", pSqlInstance.Int, data.transferId);
			...
			request.execute(
				"SONDA_SP_GET_PRICE_LIST_OUTSIDE_ROUTE",
				function (errReturn, recordsets, returnValue) {
					if (errReturn === undefined) {
						callback(data, socket, recordsets[0], recordsets[1], connection, pSqlInstance);
					} else {
						console.log(
							"Insert of Skus fail: " + errReturn.message,
							"info"
						);
					}
				}
